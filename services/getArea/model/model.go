...
func InitDb()error{
	//拼接链接字符串
	connString:=conf.MysqlName+":"+conf.MysqlPwd+"@tcp("+conf.MysqlAddr+":"+conf.MysqlPort+")/"+conf.MtSqlDB
	db,err:=gorm.Open("mysql",connString)
	if err != nil {
		fmt.Println("打开数据库失败",err)
		return err
	}
	db.DB().SetMaxIdleConns(50)//设置初始化数据库链接个数
	...
	//表的创建
	return db.AutoMigrate(new(User),new(House),new(HouseImage),new(Area),new(Facility),new(OrderHouse)).Error
